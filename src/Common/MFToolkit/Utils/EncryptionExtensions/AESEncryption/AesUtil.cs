...
public class AesUtil
{
    ...
    public static byte[] Encrypt(byte[] bytes, string skey)
    {
        ...
        var vector = MD5Util.Encrypt(skey, false, is16: true);
        var bVector = new byte[16];
        ...
        using var cryptoStream =
            new CryptoStream(memoryStream, aesAlg.CreateEncryptor(bKey, bVector), CryptoStreamMode.Write);
    ...
    public static byte[] Decrypt(byte[] bytes, string skey)
    {
        ...
        using var cryptoStream =
            new CryptoStream(memoryStream, aesAlg.CreateDecryptor(bKey, bVector), CryptoStreamMode.Read);
