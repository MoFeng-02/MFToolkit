...
public interface IMinecraftDownloadService : IDisposable
{
    ...
    /// 下载进度变化事件
    /// </summary>
    event Action<MinecraftDownloadProgress>? ProgressChanged;
    ...
    /// <param name="saveFileName">自定义名称(存储版本json和版本文件的名称，不需要后缀)</param>
    /// <param name="storageOptions">存储选项</param>
    /// <param name="priority">下载优先级</param>
    /// <param name="cancellationToken">任务令牌</param>
    /// <returns>是否成功启动</returns>
    Task<bool> StartDownloadAsync(string versionId, string? saveFileName = null, StorageOptions? storageOptions = null, DownloadPriority priority = DownloadPriority.Normal, CancellationToken cancellationToken = default);

    /// <summary>
    /// 批量启动下载任务
    /// </summary>
    /// <param name="versionIds">版本ID列表</param>
    /// <param name="saveFileNames">自定义名称列表，需要和版本Id列表下标对应(存储版本json和版本文件的名称，不需要后缀)</param>
    /// <param name="storageOptions">存储选项</param>
    /// <param name="priority">下载优先级</param>
    /// <param name="cancellationToken">任务令牌</param>
    /// <returns>成功启动的版本数量</returns>
    Task<int> StartBatchDownloadAsync(IEnumerable<string> versionIds, IEnumerable<string>? saveFileNames = null, StorageOptions? storageOptions = null, DownloadPriority priority = DownloadPriority.Normal, CancellationToken cancellationToken = default);
    ...
    Task<MinecraftValidationFileResult> ValidateGameFilesAsync(string versionId, StorageOptions storageOptions, CancellationToken cancellationToken = default);
    ...
    Task<bool> IsVersionInstalledAsync(string versionId, StorageOptions storageOptions, CancellationToken cancellationToken = default);
    ...
    Task<bool> CanResumeDownloadAsync(string versionId, StorageOptions storageOptions, CancellationToken cancellationToken = default);
