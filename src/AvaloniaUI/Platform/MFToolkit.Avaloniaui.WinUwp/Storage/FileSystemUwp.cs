...
public class FileSystemUwp : FileSystem
{
    ...
    static string AppSpecificPath =>
            Path.Combine(CleanPath(AppInfoImplementation.PublisherName), CleanPath(Platform.AppInfo.AppInfo.PackageName));

    protected override string AppDataDirectoryMethod() =>
            Path.Combine(CleanPath(AppInfoImplementation.PublisherName), CleanPath(Platform.AppInfo.AppInfo.PackageName));

    protected override string CacheDirectoryMethod() => AppInfoUtils.IsPackagedApp
